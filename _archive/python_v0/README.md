...
- **NotebookLM自動同期**: 処理済みファイルのOCRテキストを年度別累積ドキュメント（Markdown形式のGoogleドキュメント）に自動追記
...
- **v2.1.0 (2026-01-31)**:
  - **NotebookLM Sync (Markdown)**: 累積ドキュメントをMarkdown記法を使用したGoogleドキュメントに変更し、AI可読性を向上
  - **Storage Optimization**: サービスアカウントのストレージ制限回避ロジックを実装
  - **Bug Fix**: 共有フォルダ内の重複作成問題を修正
- **v2.0.0 (2026-01-30)**: Full Cloud Run Migration
